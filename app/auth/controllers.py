...
class AuthControllers(BaseControllers):
    ...
    async def register_user(self, data: schemas.RegisterRequest, commons: CommonsDependencies) -> dict:
        data = data.model_dump()
        user = await user_controllers.register(fullname=data["fullname"], email=data["email"], password=data["password"], phone_number=data.get("phone_number"), commons=commons)
        # Generate an access token for the user.
        user["access_token"] = await self.service.create_access_token(user_id=user["_id"], user_type=user["type"])
        user["token_type"] = "bearer"
        return user

    async def login_user(self, data: schemas.LoginRequest, commons: CommonsDependencies) -> dict:
        data = data.model_dump()
