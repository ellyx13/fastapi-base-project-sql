...
class AuthControllers(BaseControllers):
    ...
    async def register_user(self, data: schemas.RegisterRequest, commons: CommonsDependencies) -> schemas.LoginResponse:
        ...
        user_dict["access_token"] = await self.service.create_access_token(user_id=user.id, user_type=user.type)
        return schemas.LoginResponse.model_validate(user_dict)
